...
|-- THIRD_PARTY_LICENSES.md  # GPL/LGPL/MIT components in the bundle
|-- PRIVACY.md          # Privacy policy (for Microsoft Partner Center)
...
This project's own code (`main.py`, `uxplay_runner.py`, `fullscreen.py`, `i18n.py`, `user_settings.py`, `config.py`, `launcher.c`, `install.ps1`, `scripts/`, `icon.svg`) is released under the **MIT License** — see [`LICENSE`](LICENSE).

The combined **installer bundle** redistributes third-party software (notably UxPlay under GPL v3, GStreamer under LGPL v2.1, and various permissively-licensed codec libraries). The bundle as a whole is therefore subject to GPL v3 redistribution obligations. See [`THIRD_PARTY_LICENSES.md`](THIRD_PARTY_LICENSES.md) for the complete component list, license texts, and source links.
